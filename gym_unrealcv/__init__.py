...
# Custom: Driving
for env in ['CitySim1']:
    setting_file = 'driving/{env}.json'.format(env=env)
    settings = load_env_setting(setting_file)
    for i, reset in enumerate(['onepoint', 'random', 'waypoint', 'testpoint']):
        for action in ['Discrete', 'Continuous']:  # action type
            for obs in ['Color', 'Depth', 'Rgbd']:  # observation type
                for category in settings['targets']:
                    register(
                        id='UnrealDrive-{env}{category}-{action}{obs}-v{reset}'.format(env=env, category=category, action=action, obs=obs, reset=i),
                        entry_point='gym_unrealcv.envs:UnrealCvDrive',
                        kwargs={'setting_file': 'driving/{env}.json'.format(env=env),
                                'category': category,
                                'reset_type': reset,
                                'action_type': action,
...
# -------------------------------------------------
for env in ['RealisticRoom', 'Arch1', 'Arch3', 'UrbanCity', 'Basic1']:
