...
class ResetPoint():
    def __init__(self, setting, type, init_pose):
        ...
        elif self.reset_type == 'onepoint':
            self.onepoint = setting['onepoint']
    ...
    def select_resetpoint(self):
        ...
        elif 'onepoint' in self.reset_type:
            current_pose = self.reset_onepoint()
        return current_pose

    def reset_onepoint(self):
        # Reset from a single pose
    ...
    def select_waypoint_times(self):
        ...
        self.start_id = random.randint(0, int((len(self.waypoints) - 1) / 3))
